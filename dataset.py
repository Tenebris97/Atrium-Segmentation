...
class CardiacDataset(torch.utils.data.Dataset):
    ...
    def change_img_to_lbl_path(path):
        # Get all directories within the path
        parts = list(path.parts)
        # Replace imagesTr with labelsTr
        parts[parts.index('data')] = 'mask'
        # Combine list back into a Path object
        return Path(*parts)
    ...
    def augment(self, slice, mask):
        ...
        slice_aug, mask_aug = self.augment_params(
            image=slice, segmentation_maps=mask)
    ...
    def __getitem__(self, idx):
        ...
        mask_path = self.change_img_to_lbl_path(file_path)
